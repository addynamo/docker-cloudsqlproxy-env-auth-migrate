...
flyway -locations="filesystem:${MIGRATION_SQL_PATH}" -url=jdbc:mysql://127.0.0.1:3306/$DATABASE_NAME?useSSL=false -user="$DATABASE_USER" -password="$DATABASE_PASSWORD" migrate
if [ $? -ne 0 ]; then
